...
class NewsRepository {
  ...
  Future<List<News>> getNews({String? schoolType = 'ballet', int? cityId =1, int? count = 10  }) async {
    List<News> list = <News>[];
    
    final iterableNews = await _newsApi.fetchNews(schoolType: schoolType!, cityId: cityId!, count: count!).onError((error, stackTrace) {return error;});
    for (api.News apiNews in iterableNews) {
      if (apiNews.language == 'ru' || apiNews.language == null){
        List<String> additionalImagePathsList = [];
         if (apiNews.additionalImagesPaths != null && apiNews.additionalImagesPaths!.isNotEmpty){
